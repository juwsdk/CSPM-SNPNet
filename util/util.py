...
def visualize(image_name, predictions, weight_name,file='runs/vis_results/pred_' ,name='mm'):
    palette = get_palette()
...
def compute_results(conf_total):
    ...
    if consider_unlabeled:
        start_index = 0  # Consider unlabeled, start from index 0
    else:
        start_index = 1  # Skip unlabeled, start from index 1
    ...
    for cid in range(start_index, n_class):
        # Precision: TP / (TP + FP)
        if conf_total[:, cid].sum() == 0:
            precision_per_class[cid] = np.nan
        else:
            precision_per_class[cid] = float(conf_total[cid, cid]) / float(conf_total[:, cid].sum())

        # Recall: TP / (TP + FN)
        if conf_total[cid, :].sum() == 0:
            recall_per_class[cid] = np.nan
        else:
            recall_per_class[cid] = float(conf_total[cid, cid]) / float(conf_total[cid, :].sum())

        # IoU: TP / (TP + FP + FN)
        intersection = conf_total[cid, cid]
        union = conf_total[cid, :].sum() + conf_total[:, cid].sum() - intersection
        if union == 0:
            iou_per_class[cid] = np.nan
        else:
            iou_per_class[cid] = float(intersection) / float(union)
