...
class PST_dataset(Dataset):
    ...
    def read_image(self, name, folder):
        file_path = os.path.join(self.data_dir, self.split, folder, name)
    ...
    def __getitem__(self, index):
        ...
        rgb = self.read_image(name, 'rgb')
        the = self.fill_thermal(self.read_image(name, 'thermal'))
        ...
        if self.is_sup:
            ...
            label = np.asarray(
                PIL.Image.fromarray(label).resize((self.input_w, self.input_h), resample=PIL.Image.NEAREST))
