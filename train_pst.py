...
def train(epo, model, train_loader, optimizer, batch_size):
    ...
    for it, (images, labels, names) in enumerate(train_loader):
        ...
        print('Train: %s, epo %s/%s, iter %s/%s, lr %.8f, %.2f img/sec, loss %.4f (rgb=%.4f  thermal=%.4f), time %s' \
            % (args.model_name, epo, args.epoch_max, it+1, len(train_loader), lr_this_epo, len(names)/(time.time()-start_t), float(loss), float(loss_rgb), float(loss_thermal),
...
def testing(epo, model, test_loader):
    ...
    label_list = ["unlabeled", "hand_drill", "backpack", "fire_extinguisher", "survivor"]
    ...
    if epo==0:
        with open(testing_results_file, 'w') as f:
            ...
            f.write("# epoch: unlabeled, hand_drill, backpack, fire_extinguisher, survivor, average(nan_to_num). (IoU %)\n")
...
if __name__ == '__main__':
    ...
    if os.path.exists("./runs_pst"):
        shutil.rmtree("./runs_pst")
    weight_dir = os.path.join("./runs_pst", args.model_name)
    os.makedirs(weight_dir)
 
    writer = SummaryWriter("./runs_pst/tensorboard_log")
    ...
    train_dataset = PST_dataset(data_dir=args.data_dir, split='train', transform=augmentation_methods)
    test_dataset = PST_dataset(data_dir=args.data_dir, split='test')
