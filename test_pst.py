...
parser.add_argument('--model_dir', '-wd', type=str, default='./runs_pst/CSPM_SNP')
...
if __name__ == '__main__':
    ...
    if os.path.exists("./runs_pst"):
        os.makedirs("./runs_pst/vis_results")
    else:
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            # save demo images
            visualize(image_name=names, predictions=logits_rgb.argmax(1), weight_name=args.weight_name, name='rgb',file='runs_pst/vis_results/pred_')
            visualize(image_name=names, predictions=logits_thermal.argmax(1), weight_name=args.weight_name, name='thermal',file='runs_pst/vis_results/pred_')
            visualize(image_name=names, predictions=torch.tensor(label).cuda().reshape(1,720,1280), weight_name=args.weight_name, name='gt',file='runs_pst/vis_results/pred_')
    ...
    conf_total_matfile = os.path.join("./runs_pst", 'conf_'+args.weight_name+'.mat')
